...
// Debounce viewport-driven fetches and cache by normalized bbox so rapid panning
// doesn't hammer the API (matches the reports map cadence).
const VIEWPORT_DEBOUNCE_MS = 250;
const IMAGERY_CACHE_TTL_MS = 45_000;
...
// Per-building damage classes (Microsoft AI for Good footprints). Colors must
// match DAMAGE_COLORS in BuildingsLayer.tsx.
const buildingDamageLegendItems: { label: string; color: string }[] = [
  { label: "Pérdida total (80-100%)", color: "#7f1d1d" },
  { label: "Daño severo (60-80%)", color: "#b53a24" },
  { label: "Daño moderado (40-60%)", color: "#b45309" },
  { label: "Daño leve (20-40%)", color: "#8a6a24" },
];

function BuildingDamageLegend() {
  return (
    <div
      className="map-legend damage-zone-legend"
      aria-label="Leyenda de daño por edificio"
    >
      <p>Daño por edificio</p>
      {buildingDamageLegendItems.map((item) => (
        <div key={item.label}>
          <span
            className="damage-zone-swatch building-damage-swatch"
            style={{ "--zone-color": item.color } as CSSProperties}
            aria-hidden="true"
          />
          {item.label}
      ...
      >
        <strong>Fuente:</strong> {damageZoneSourceLabel("ms-ai-for-good")}
      </p>
    </div>
  );
}
...
export function DamageMapWithZonesToggle({
  ...
  const [showDamageBuildings, setShowDamageBuildings] = useState(true);
  ...
  useEffect(() => {
    ...
    const key = normalizeRequestKey("/api/imagery", new URLSearchParams(bounds));
    const timeout = window.setTimeout(() => {
      cachedJson<{ scenes: ImageryScene[] }>(
        key,
        IMAGERY_CACHE_TTL_MS,
        () =>
          fetch(`/api/imagery?${bounds}`, { signal: controller.signal }).then(
            (r) => (r.ok ? r.json() : { scenes: [] }),
          ),
        controller.signal,
      )
        .then((data) => {
  ...
  return (
    <>
      <DamageMapClient
        ...
        showDamageBuildings={showDamageBuildings}
      ...
      <div className="map-overlay-controls">
        <div className="map-overlay-stack">
          ...
          <Button
            type="button"
            variant={showDamageBuildings ? "secondary" : "outline"}
            size="sm"
            className="map-zones-toggle-button"
            onClick={() => setShowDamageBuildings((v) => !v)}
            aria-pressed={showDamageBuildings}
            aria-label={
              showDamageBuildings
                ? "Ocultar daño por edificio"
                : "Mostrar daño por edificio"
            }
          >
            {showDamageBuildings ? (
              <>
          ...
          {showDamageBuildings ? <BuildingDamageLegend /> : null}
          <Button
            type="button"
